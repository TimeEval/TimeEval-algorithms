...
class Trainer(VarScopeObject):
    ...
    def fit(self, values,
            valid_portion=0.3, summary_dir=None, with_stats=True, save_model_fn=lambda: None):
        """
        ...
        if valid_portion > 0.:
            print(f"Train-val split: {valid_portion}")
        ...
        with TrainLoop(
            ...
            def make_checkpoint(loss):
                print(f"Should I create a checkpoint with {loop._best_valid_metric}/{loss} (old/new val loss)? ", end="")
                if (
                    loop._best_valid_metric is None or
                    (loop._valid_metric_smaller_is_better and loss < loop._best_valid_metric) or
                    (not loop._valid_metric_smaller_is_better and loss > loop._best_valid_metric)
                   ):
                    print("Yes - Checkpoint created!")
                    save_model_fn()
                else:
                    print("No")
            ...
            def validate():
                if valid_portion > 0:
                    ...
                    with loop.timeit('valid_time'), loop.metric_collector('valid_loss') as mc:
                        ...
                        # create model checkpoint before updating the loop (by __exit__)
                        make_checkpoint(loss)
