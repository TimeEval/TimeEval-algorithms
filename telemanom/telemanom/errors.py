...
class Errors:
    def __init__(self, channel, config, run_id, save: bool = True):
        """
        ...
        self.config = config
        self.window_size = self.config.smoothing_window_size
        ...
        smoothing_window = int(self.config.batch_size * self.config.smoothing_window_size
        ...
        if not channel.id == 'C-2':  # anomaly occurs early in window
            self.e_s[:self.config.window_size] = \
                [np.mean(self.e_s[:self.config.window_size * 2])] * self.config.window_size

        if save:
            self.save_errors(run_id, channel.id)
    ...
    @staticmethod
    def _get_error_path(run_id, channel_id):
        return os.path.join('data', run_id, 'smoothed_errors', '{}.npy'.format(channel_id))

    def save_errors(self, run_id, channel_id):
        np.save(self._get_error_path(run_id, channel_id), np.array(self.e_s))

    @staticmethod
    def load_errors(run_id, channel_id):
        return np.load(Errors._get_error_path(run_id, channel_id))
    ...
    def process_batches(self, channel):
        ...
        for i in range(0, self.n_windows+1):
            ...
            idx = (self.config.smoothing_window_size * self.config.batch_size) \
        ...
        if len(self.i_anom) > 0:
            ...
            self.E_seq = [(e_seq[0] + self.config.window_size,
                           e_seq[1] + self.config.window_size) for e_seq in self.E_seq]
...
class ErrorWindow:
    def __init__(self, channel, config, start_idx, end_idx, errors, window_num):
        ...
        self.num_to_ignore = self.config.window_size * 2
        # if y_test is small, ignore fewer
        if len(channel.y_test) < 2500:
            self.num_to_ignore = self.config.window_size
