...
class Channel:
    ...
    def shape_data(self, arr, train=True):
        ...
        data = []
        for i in range(len(arr) - self.config.window_size - self.config.prediction_window_size):
            data.append(arr[i:i + self.config.window_size + self.config.prediction_window_size])
        data = np.array(data)
        ...
        if train:
            ...
            self.y_train = data[:, -self.config.prediction_window_size:, 0]  # telemetry value is at position 0
        else:
            self.X_test = data[:, :-self.config.prediction_window_size, :]
            self.y_test = data[:, -self.config.prediction_window_size:, 0]  # telemetry value is at position 0
