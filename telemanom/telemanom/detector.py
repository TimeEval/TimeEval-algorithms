...
class Detector:
    def __init__(self, config: Config, labels_path=None, result_path='results/', model_path: Optional[os.PathLike] = None):
        """
        ...
        self.result_df = None
        self.chan_df = None
        self.model_path = model_path or os.path.join("data", self.config.use_id, "models")
        ...
        if model_path:
            ...
            hdlr = logging.FileHandler('%s.log' % self.id)
        else:
    ...
    def predict(self, channels: List[Channel]) -> List[np.ndarray]:
        error_list = []
        for channel in channels:
            model = Model(self.config, self.id, channel, model_path=self.model_path)
            channel = model.batch_predict(channel, save_y_hat=False)
            errors = Errors(channel, self.config, self.id, save=False)
            error_list.append(errors.e_s)
        return error_list

    def run(self, with_threshold: bool = True):
        """
    ...
    def load_errors(self, run_id, channel_id):
        return Errors.load_errors(run_id, channel_id)
